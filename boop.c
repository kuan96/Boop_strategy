...
int board[BOUNDARY][BOUNDARY] = {0};
int priority[BOUNDARY][BOUNDARY] = {0};
int myColor = 0;
int CatOrKitten = 0, PutLocation = 0, TakeLocation = 0;

void swap_arr(int arr1[BOUNDARY][BOUNDARY], int arr2[BOUNDARY][BOUNDARY])
{
...
void compute_priority()
{
    ...
    // 每一格都走看看
    int tmp_arr[BOUNDARY][BOUNDARY] = {0};
    ...
    for (int i = 0; i < BOUNDARY; ++i)
    {
        for (int j = 0; j < BOUNDARY; ++j)
        {
            if (board[i][j] == 0)
            {
                ...
                push_cat(i, j, CatOrKitten * 2);

                if (check_horizontal() || check_vertical() || check_slash())
                {
                    priority[i][j] += 25;
                }

                swap_arr(board, tmp_arr);
            }
        }
    }
}
